...
export function Services({ standalone = false }: ServicesProps) {
  ...
  return (
    <section
      id={standalone ? undefined : "services"}
      className={`relative bg-ink text-cream ${standalone ? "pb-28 md:pb-40" : "py-28 md:py-40"}`}
    >
      ...
      <div className="relative mx-auto max-w-[1400px] px-6 md:px-10">
        ...
        <div className={`divide-y divide-cream/15 border-y border-cream/15 ${standalone ? "" : "mt-20"}`}>
          {services.map((s, i) => (
            <Reveal key={s.title} delay={i * 100}>
              ...
              >
                ...
                <div className="col-span-12 md:col-span-5">
                  <p className={bodyCopyDarkLarge}>{s.body}</p>
                </div>
                <div className="col-span-12 flex items-center justify-end md:col-span-1">
                  <span className="text-2xl transition-transform duration-500 group-hover:rotate-45" style={{ fontFamily: "sans-serif" }}>
                    +
                  </span>
                </div>
              </div>
