...
@section('content')
    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            @if ($unreadCount > 0)
                <p class="mt-1 text-sm text-amber-700">{{ $unreadCount }} unread {{ str('request')->plural($unreadCount) }}</p>
            @endif
        </div>
        <div class="flex flex-wrap gap-3">
            @if ($unreadCount > 0)
                <form method="POST" action="{{ route('admin.estimate-requests.mark-all-read') }}">
                    @csrf
                    @method('PATCH')
    ...
    <div class="mb-4 flex gap-2">
        <a
            href="{{ route('admin.estimate-requests.index', ['filter' => 'all']) }}"
        ...
        <a
            href="{{ route('admin.estimate-requests.index', ['filter' => 'unread']) }}"
                        ...
                        <tr class="{{ $item->isUnread() ? 'bg-amber-50/40 hover:bg-amber-50/70' : 'hover:bg-gray-50' }}">
                            ...
                            <td class="whitespace-nowrap px-6 py-4 text-right text-sm">
                                <div class="flex items-center justify-end gap-3">
                                    <a href="{{ route('admin.estimate-requests.show', $item) }}" class="font-medium text-indigo-600 hover:text-indigo-900">View</a>
                                    <form action="{{ route('admin.estimate-requests.destroy', $item) }}" method="POST" class="inline" onsubmit="return confirm('Delete this estimate request?')">
                                        @csrf
                                        @method('DELETE')
