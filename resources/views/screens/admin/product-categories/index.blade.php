...
@section('content')
    ...
    @include('screens.admin.partials.page-header', [
        'title' => $title,
        'actionRoute' => route('admin.product-categories.create'),
        'actionLabel' => 'Add Category',
    ])

    <div class="mb-4 rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
        <form method="GET" action="{{ route('admin.product-categories.index') }}" class="grid grid-cols-1 gap-3 md:grid-cols-12 md:items-end">
            <div class="md:col-span-4">
                ...
                <input
                    ...
                    placeholder="Search by name, short label, or slug..."
            ...
            <div class="md:col-span-2">
                ...
                <select id="sort" name="sort" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    <option value="short_name" @selected($filters['sort'] === 'short_name')>Short label</option>
            ...
            <div class="flex gap-2 md:col-span-2">
                ...
                <a href="{{ route('admin.product-categories.index') }}" class="inline-flex items-center justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-xs font-semibold uppercase tracking-widest text-gray-700 hover:bg-gray-50">
                    Reset
                </a>
            </div>
        </form>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Short Label</th>
                ...
                <tbody class="divide-y divide-gray-200 bg-white">
                    @forelse ($items as $item)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $item->short_name ?: '—' }}</td>
                            <td class="px-6 py-4 font-mono text-xs text-gray-500">{{ $item->slug }}</td>
                            <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-600">{{ $item->products_count }}</td>
                            ...
                            <td class="whitespace-nowrap px-6 py-4 text-right text-sm">
                                <x-admin.row-actions
                                    :edit-route="route('admin.product-categories.edit', $item)"
                                    :destroy-route="route('admin.product-categories.destroy', $item)"
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-8 text-center text-sm text-gray-500">No categories found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
