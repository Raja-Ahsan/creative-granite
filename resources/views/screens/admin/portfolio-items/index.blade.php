...
@section('content')
    ...
    @include('screens.admin.partials.page-header', [
        'title' => $title,
        'actionRoute' => route('admin.portfolio-items.create'),
        'actionLabel' => 'Add Item',
    ])

    <div class="mb-4 rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
        <form method="GET" action="{{ route('admin.portfolio-items.index') }}" class="grid grid-cols-1 gap-3 md:grid-cols-12 md:items-end">
            ...
            <div class="flex gap-2 md:col-span-2">
                ...
                <a href="{{ route('admin.portfolio-items.index') }}" class="inline-flex items-center justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-xs font-semibold uppercase tracking-widest text-gray-700 hover:bg-gray-50">
                    Reset
                </a>
            </div>
        </form>
                    ...
                    @forelse ($items as $item)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="whitespace-nowrap px-6 py-4 text-right text-sm">
                                <x-admin.row-actions
                                    :edit-route="route('admin.portfolio-items.edit', $item)"
                                    :destroy-route="route('admin.portfolio-items.destroy', $item)"
                                    confirm="Delete this gallery item?"
                                />
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-8 text-center text-sm text-gray-500">No gallery items found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
