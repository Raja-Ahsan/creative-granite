...
@section('content')
    ...
    @include('screens.admin.partials.page-header', [
        'title' => $title,
        'actionRoute' => route('admin.instagram-posts.create'),
        'actionLabel' => 'Add Image',
    ])

    <div class="mb-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
        Homepage shows up to <strong>12 featured</strong> images.
        Currently featured: <strong>{{ $featuredCount }}</strong> / 12.
    </div>

    <div class="mb-4 rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
        <form method="GET" action="{{ route('admin.instagram-posts.index') }}" class="grid grid-cols-1 gap-3 md:grid-cols-12 md:items-end">
            ...
            <div class="flex gap-2 md:col-span-2">
                ...
                <a href="{{ route('admin.instagram-posts.index') }}" class="inline-flex items-center justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-xs font-semibold uppercase tracking-widest text-gray-700 hover:bg-gray-50">
                    Reset
                </a>
            </div>
        </form>
                    ...
                    @forelse ($items as $item)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="whitespace-nowrap px-6 py-4 text-right text-sm">
                                <x-admin.row-actions
                                    :edit-route="route('admin.instagram-posts.edit', $item)"
                                    :destroy-route="route('admin.instagram-posts.destroy', $item)"
                                    confirm="Delete this Instagram image?"
                                />
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-8 text-center text-sm text-gray-500">No Instagram images yet. Add and mark up to 12 as featured.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
